...
const result={matches:m.length,done,live,upcoming,unique:new Set(ids).size,dup,byGroup,ko:ko.length,koPredDraw,tbd,scorers:Object.keys(scorers).length,top,logFirst,marker0837:(html.match(/marker:cron-full-check-20260912-0837/g)||[]).length,bodyHasEnded,champion,progress};
console.log(JSON.stringify(result, null, 2));
if(m.length!==104||done!==104||live||upcoming||new Set(ids).size!==104||dup.length||ko.length!==32||koPredDraw.length||tbd.length||Object.keys(scorers).length!==44||logFirst!=='2026-09-12 08:37'||!bodyHasEnded||!progress) process.exit(1);
