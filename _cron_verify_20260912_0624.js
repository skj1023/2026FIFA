...
(async()=>{
  ...
  const out={
    ...
    marker0415:(html.match(/marker:cron-full-check-20260912-0415/g)||[]).length,
    marker0254:(html.match(/marker:cron-full-check-20260912-0254/g)||[]).length
  };
  console.log(JSON.stringify(out,null,2));
  dom.window.close();
  const okGroups = 'ABCDEFGHIJKL'.split('').every(g=>byGroup[g]===6) && byGroup.KO===32;
  if(errors.length||out.matchCount!==104||out.uniqueIds!==104||out.done!==104||out.live!==0||out.upcoming!==0||!okGroups||out.koDrawPred.length||out.marker0624!==1||out.marker0415!==1||out.marker0254!==1||out.logFirst!=='2026-09-12 06:24'||!out.bodyHasEnded||!out.championSpain||out.bracketTBD!==0) process.exit(1);
  process.exit(0);
})();
